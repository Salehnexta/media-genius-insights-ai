...
export class EnhancedScrapingService {
  ...
  async scrapeAndAnalyze(url: string): Promise<ScrapingResult> {
    ...
    try {
      ...
      const rawContent = scrapingResult.data?.content || '';
  ...
  private async analyzeWithAI(content: string, url: string): Promise<string> {
    try {
...
أجب باللغة العربية وكن مفصلاً ومفيداً.`;
  ...
  private async generateQuestions(content: string, url: string): Promise<string> {
    try {
...
ضع كل سؤال في سطر منفصل مع رقم.`;
  ...
  private async extractStructuredData(content: string, url: string): Promise<any> {
    try {
